...
public class MinFilter extends JFrame {
	 ...
	 */
	public MinFilter(String str_SaltAndPepperString) {
		setTitle("\u6700\u5C0F\u503C\u6FFE\u6CE2\u5668( Min Filter )");
		...
		JLabel lblNewLabel = new JLabel("\u6700\u5C0F\u503C\u6FFE\u6CE2\u5668( Min Filter )");
		...
		tf_FileName.setText("MinFilter.jpg");
		...
		btn_Process.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					mfImg = ImgFunction.MinFilter(new File(str_SaltAndPepperString), Integer.parseInt(lb_WindowSize.getText()));
					Image img2 = mfImg.getScaledInstance(700, 500, Image.SCALE_AREA_AVERAGING);
					ImageIcon icon= new ImageIcon(img2);
					lb_MinFilterImg.setIcon(icon);
					btn_Save.setEnabled(true);
					tf_FileName.setText(String.format("MinFilter_%s.jpg", lb_WindowSize.getText()));
				} catch (IOException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
